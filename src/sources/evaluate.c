...
// Special eval terms
const Scorepair Initiative = SPAIR(25, 32);

// Knight eval terms
const Scorepair KnightShielded = SPAIR(  5,  21);
const Scorepair KnightOutpost  = SPAIR( 33,  29);

const Scorepair ClosedPosKnight[5] = {
    SPAIR(  8, -17), SPAIR(  8,   5), SPAIR( 10,  22), SPAIR( 15,  31),
    SPAIR( 16,  48)
};

// Bishop eval terms
const Scorepair BishopPairBonus    = SPAIR( 22,  94);
const Scorepair BishopShielded     = SPAIR(  1,   1);
const Scorepair BishopOutpost      = SPAIR( 49,  23);
const Scorepair BishopLongDiagonal = SPAIR( 13,  24);

const Scorepair BishopPawnsSameColor[7] = {
    SPAIR( 16,  39), SPAIR( 16,  29), SPAIR( 13,  20), SPAIR( 10,  13),
    SPAIR(  6,   5), SPAIR(  4,  -1), SPAIR( -2, -10)
};

// Rook eval terms
const Scorepair RookOnSemiOpenFile = SPAIR( 19,  17);
const Scorepair RookOnOpenFile     = SPAIR( 35,  13);
...
const Scorepair RookBuried         = SPAIR(-70, -35);

// Mobility eval terms
const Scorepair KnightMobility[9] = {
    SPAIR( -55,   16), SPAIR( -44,  -30), SPAIR( -34,   36), SPAIR( -25,   58),
    SPAIR( -17,   77), SPAIR( -12,   97), SPAIR(  -6,  107), SPAIR(   2,  113),
    SPAIR(   7,  107)
};

const Scorepair BishopMobility[14] = {
    SPAIR( -56,  -48), SPAIR( -44,  -40), SPAIR( -27,  -17), SPAIR( -24,   15),
    SPAIR( -16,   33), SPAIR( -12,   48), SPAIR(  -9,   58), SPAIR(  -7,   63),
    SPAIR(  -6,   66), SPAIR(  -4,   70), SPAIR(   0,   63), SPAIR(   9,   60),
    SPAIR(  10,   56), SPAIR(  23,   48)
};

const Scorepair RookMobility[15] = {
    SPAIR( -97,  -29), SPAIR( -37,   42), SPAIR( -25,   79), SPAIR( -29,   95),
    SPAIR( -27,  106), SPAIR( -31,  118), SPAIR( -32,  127), SPAIR( -26,  131),
    SPAIR( -23,  140), SPAIR( -14,  148), SPAIR( -13,  155), SPAIR(  -8,  161),
    SPAIR(  -1,  163), SPAIR(   9,  166), SPAIR(  26,  159)
};

const Scorepair QueenMobility[28] = {
    SPAIR( -55, -143), SPAIR(  25,  185), SPAIR(   5,  154), SPAIR(  -2,  119),
    SPAIR(   5,   76), SPAIR(   2,  119), SPAIR(   1,  152), SPAIR(   1,  177),
    SPAIR(   2,  187), SPAIR(   2,  207), SPAIR(   4,  216), SPAIR(   7,  220),
    SPAIR(   7,  230), SPAIR(  10,  233), SPAIR(  10,  238), SPAIR(  10,  244),
    SPAIR(  12,  244), SPAIR(  15,  241), SPAIR(  21,  232), SPAIR(  21,  232),
    SPAIR(  41,  207), SPAIR(  42,  201), SPAIR(  42,  191), SPAIR(  27,  171),
    SPAIR(  48,  157), SPAIR(  -3,  167), SPAIR(  14,  178), SPAIR(  44,  155)
};

// King Safety linear eval terms
const Scorepair FarKnight = SPAIR(-23, -12);
const Scorepair FarBishop = SPAIR( -8,  -9);
const Scorepair FarRook   = SPAIR(-11,   6);
const Scorepair FarQueen  = SPAIR( -7,  11);

// King Safety eval terms
const Scorepair KnightWeight    = SPAIR(  47,   31);
const Scorepair BishopWeight    = SPAIR(  33,   55);
const Scorepair RookWeight      = SPAIR(  36,  -41);
const Scorepair QueenWeight     = SPAIR(  11,    4);
const Scorepair AttackWeight    = SPAIR(   8,   18);
const Scorepair WeakKingZone    = SPAIR(  27,  -48);
const Scorepair SafeKnightCheck = SPAIR(  77,   15);
const Scorepair SafeBishopCheck = SPAIR(  38,   62);
const Scorepair SafeRookCheck   = SPAIR(  93,   77);
const Scorepair SafeQueenCheck  = SPAIR(  48,  106);
const Scorepair UnsafeCheck     = SPAIR(  18,   75);
const Scorepair QueenlessAttack = SPAIR( -85,  -22);
const Scorepair SafetyOffset    = SPAIR(  30,   38);
...
const Scorepair KingStorm[24] = {
    SPAIR(  -4,   -2), SPAIR( -20,   -1), SPAIR(   9,    7), SPAIR(  -8,    6),
    SPAIR( -26,   15), SPAIR( -27,    5), SPAIR( -48,   -1), SPAIR( -20,  -10),
    SPAIR(   0,    0), SPAIR(   0,   -7), SPAIR(  34,   14), SPAIR(   5,   -5),
    SPAIR(  -3,   -4), SPAIR(  -4,   11), SPAIR(   7,   29), SPAIR(  17,   -2),
    SPAIR(  14,   -1), SPAIR(   6,    0), SPAIR(  35,    3), SPAIR(  25,   -5),
    SPAIR(  -6,   -6), SPAIR( -13,   23), SPAIR(  -4,   56), SPAIR(  -1,  -34)
};

const Scorepair KingShelter[24] = {
    SPAIR( -51,    9), SPAIR( -39,   76), SPAIR( -39,  -24), SPAIR( -25,    6),
    SPAIR(   7,   -3), SPAIR(  11,   -2), SPAIR(  -5,    0), SPAIR(  -9,  -43),
    SPAIR(   0,    0), SPAIR(  -4,  -16), SPAIR(  -4,   49), SPAIR(   3,   25),
    SPAIR(  12,   11), SPAIR(  34,    0), SPAIR(   4,    0), SPAIR(  15,  -31),
    SPAIR( -30,  -23), SPAIR(  13,  -77), SPAIR(   4,   45), SPAIR(  13,   67),
    SPAIR(  14,   15), SPAIR(  20,    0), SPAIR(   8,    0), SPAIR(  17,   10)
};

// Threat eval terms
const Scorepair PawnThreats[6] = {
    SPAIR( -4,  -7), SPAIR( 72,  67), SPAIR( 70, 104),
    SPAIR( 66,  60), SPAIR( 64,  16), 0
};

const Scorepair KnightThreats[6] = {
    SPAIR(-10,   8), SPAIR(  4,   7), SPAIR( 43,  56),
    SPAIR( 91,  50), SPAIR( 46,  58), 0
};

const Scorepair BishopThreats[6] = {
    SPAIR( -3,   4), SPAIR( 15,  50), SPAIR(  4,  10),
    SPAIR( 57,  73), SPAIR( 61,  87), 0
};

const Scorepair RookThreats[6] = {
    SPAIR(-10,  15), SPAIR(  8,  28), SPAIR( 25,  21),
    SPAIR(  6,   5), SPAIR( 52,  57), 0
};

const Scorepair QueenThreats[6] = {
    SPAIR(  0,   9), SPAIR(  2,  11), SPAIR( -1,  30),
    SPAIR( -1,  -7), SPAIR(  3,   1), 0
};
