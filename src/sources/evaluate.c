...
// Special eval terms
const scorepair_t CastlingBonus = SPAIR(106, -67);
...
const scorepair_t KnightWeight    = SPAIR(  40,  15);
const scorepair_t BishopWeight    = SPAIR(  23,  25);
const scorepair_t RookWeight      = SPAIR(  18, -43);
const scorepair_t QueenWeight     = SPAIR(  24,   6);
const scorepair_t AttackWeight    = SPAIR(  20,  -1);
const scorepair_t WeakKingZone    = SPAIR(  20,  -7);
const scorepair_t SafeKnightCheck = SPAIR(  57,   7);
const scorepair_t SafeBishopCheck = SPAIR(  48,  24);
const scorepair_t SafeRookCheck   = SPAIR(  66,  34);
const scorepair_t SafeQueenCheck  = SPAIR(  46,  33);
const scorepair_t QueenlessAttack = SPAIR(-102,   3);
const scorepair_t SafetyOffset    = SPAIR(  35,  39);

const scorepair_t KingStorm[24] = {
    // Side
    SPAIR(  -7,  -1), SPAIR(  -9,  -1), SPAIR(  22,   4), SPAIR(   1,   3),
    SPAIR( -20,   9), SPAIR( -23,   4), SPAIR( -46,   4), SPAIR( -16,   6),

    // Front
    SPAIR(   0,   0), SPAIR(  -3,  -2), SPAIR(  33,  11), SPAIR(   6,   1),
    SPAIR(  -2,   2), SPAIR(  -3,  10), SPAIR(   7,  17), SPAIR(  16,   3),

    // Center
    SPAIR(   5,  -1), SPAIR(  -1,   0), SPAIR(  35,   4), SPAIR(  28,  -2),
    SPAIR(  -1,   4), SPAIR(  -9,  12), SPAIR(  -7,  27), SPAIR(   5,  -2)
};

const scorepair_t KingShelter[24] = {
    // Side
    SPAIR( -32,   8), SPAIR( -30,  35), SPAIR( -30,  -5), SPAIR( -22,   5),
    SPAIR(   9,  -1), SPAIR(   5,   0), SPAIR(   5,   0), SPAIR(  -3, -17),

    // Front
    SPAIR(   0,   0), SPAIR(  -4,   8), SPAIR(  -5,  28), SPAIR(   9,  13),
    SPAIR(  11,   4), SPAIR(  19,   0), SPAIR(   5,   0), SPAIR(  22, -13),

    // Center
    SPAIR( -33,  -6), SPAIR(   4, -25), SPAIR(  10,  19), SPAIR(  13,  32),
    SPAIR(  19,   8), SPAIR(  22,   0), SPAIR(   5,   0), SPAIR(  20,  11)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  6, 18);
const scorepair_t KnightOutpost       = SPAIR( 10,-19);
const scorepair_t KnightCenterOutpost = SPAIR( 18,  8);
const scorepair_t KnightSolidOutpost  = SPAIR( 16, 22);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(  -2, -25), SPAIR(   7,  -6), SPAIR(   7,   9), SPAIR(  10,  19),
    SPAIR(   8,  36)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 28, 96);
const scorepair_t BishopShielded     = SPAIR(  8, 11);
const scorepair_t BishopLongDiagonal = SPAIR( 13, 29);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(  16,  29), SPAIR(  17,  19), SPAIR(  12,  10), SPAIR(   8,   2),
    SPAIR(   1,  -7), SPAIR(  -5, -14), SPAIR( -11, -19)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 12, 29);
const scorepair_t RookOnOpenFile     = SPAIR( 39, 10);
const scorepair_t RookOnBlockedFile  = SPAIR( -7,-14);
const scorepair_t RookXrayQueen      = SPAIR( 11,  4);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -59,  23), SPAIR( -43, -37), SPAIR( -35,  27), SPAIR( -28,  56),
    SPAIR( -20,  71), SPAIR( -15,  89), SPAIR(  -9,  97), SPAIR(  -3, 103),
    SPAIR(  -2,  97)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -46, -30), SPAIR( -45, -47), SPAIR( -28, -15), SPAIR( -24,  17),
    SPAIR( -16,  36), SPAIR( -11,  49), SPAIR(  -6,  56), SPAIR(  -6,  59),
    SPAIR(  -7,  59), SPAIR(  -6,  56), SPAIR(  -4,  50), SPAIR(  -0,  46),
    SPAIR(   4,  38), SPAIR(   8,  30)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-104,   4), SPAIR( -47,  25), SPAIR( -35,  69), SPAIR( -39,  88),
    SPAIR( -33, 100), SPAIR( -30, 113), SPAIR( -27, 126), SPAIR( -20, 129),
    SPAIR( -15, 137), SPAIR(  -8, 141), SPAIR(  -5, 145), SPAIR(  -2, 148),
    SPAIR(   4, 147), SPAIR(   9, 143), SPAIR(  14, 136)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -72,-159), SPAIR(  19, 165), SPAIR(  -3, 165), SPAIR(  -8, 140),
    SPAIR(  -3,  92), SPAIR(  -6, 118), SPAIR(  -4, 145), SPAIR(  -3, 168),
    SPAIR(   0, 184), SPAIR(   4, 198), SPAIR(   7, 207), SPAIR(  11, 211),
    SPAIR(  13, 215), SPAIR(  15, 218), SPAIR(  16, 221), SPAIR(  16, 226),
    SPAIR(  17, 225), SPAIR(  18, 222), SPAIR(  23, 214), SPAIR(  31, 203),
    SPAIR(  36, 199), SPAIR(  34, 190), SPAIR(  28, 182), SPAIR(  22, 173),
    SPAIR(  42, 154), SPAIR(  -8, 167), SPAIR(  13, 179), SPAIR(  47, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 47, 75);
const scorepair_t PawnAttacksRook   = SPAIR( 59, 55);
const scorepair_t PawnAttacksQueen  = SPAIR( 51, 11);
const scorepair_t MinorAttacksRook  = SPAIR( 61, 43);
const scorepair_t MinorAttacksQueen = SPAIR( 45, 52);
const scorepair_t RookAttacksQueen  = SPAIR( 54, 40);
