...
const scorepair_t KnightWeight    = SPAIR(  39,   9);
const scorepair_t BishopWeight    = SPAIR(  25,  18);
const scorepair_t RookWeight      = SPAIR(  19, -37);
const scorepair_t QueenWeight     = SPAIR(  23,   5);
const scorepair_t AttackWeight    = SPAIR(  19, -26);
const scorepair_t WeakKingZone    = SPAIR(  19, -16);
const scorepair_t SafeKnightCheck = SPAIR(  56,   4);
const scorepair_t SafeBishopCheck = SPAIR(  43,  16);
const scorepair_t SafeRookCheck   = SPAIR(  67,  21);
const scorepair_t SafeQueenCheck  = SPAIR(  44,  24);
const scorepair_t QueenlessAttack = SPAIR( -92,  -1);
const scorepair_t SafetyOffset    = SPAIR(  35,  26);
...
const scorepair_t KingStorm[24] = {
    SPAIR(  -6,  -0), SPAIR(  -6,  -0), SPAIR(  19,   2), SPAIR(   2,   2),
    SPAIR( -19,   7), SPAIR( -25,   3), SPAIR( -48,   4), SPAIR( -15,   2),
    SPAIR(   0,   0), SPAIR(  -5,  -1), SPAIR(  33,   7), SPAIR(   9,   0),
    SPAIR(  -2,   1), SPAIR(  -3,   9), SPAIR(   5,  11), SPAIR(  16,   0),
    SPAIR(   5,  -0), SPAIR(  -1,   0), SPAIR(  34,   3), SPAIR(  30,  -1),
    SPAIR(  -1,   2), SPAIR(  -8,   8), SPAIR(  -9,  20), SPAIR(   4,  -4)
};

const scorepair_t KingShelter[24] = {
    SPAIR( -34,   4), SPAIR( -32,  25), SPAIR( -29,  -2), SPAIR( -22,   3),
    SPAIR(  10,  -0), SPAIR(   7,  -0), SPAIR(   4,  -0), SPAIR(  -3, -12),
    SPAIR(   0,   0), SPAIR(  -2,   6), SPAIR(  -5,  19), SPAIR(   8,   8),
    SPAIR(  11,   2), SPAIR(  18,   0), SPAIR(   4,   0), SPAIR(  23, -10),
    SPAIR( -32,  -4), SPAIR(   5, -16), SPAIR(  10,  13), SPAIR(  14,  21),
    SPAIR(  17,   5), SPAIR(  21,   0), SPAIR(   4,   0), SPAIR(  20,   8)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  5, 19);
const scorepair_t KnightOutpost       = SPAIR(  8,-17);
const scorepair_t KnightCenterOutpost = SPAIR( 17,  9);
const scorepair_t KnightSolidOutpost  = SPAIR( 13, 20);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(  -1, -25), SPAIR(   3,  -6), SPAIR(   5,   7), SPAIR(   8,  16),
    SPAIR(   6,  35)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 23, 99);
const scorepair_t BishopShielded     = SPAIR(  6, 11);
const scorepair_t BishopLongDiagonal = SPAIR( 10, 31);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(  16,  31), SPAIR(  15,  21), SPAIR(  11,  10), SPAIR(   6,   1),
    SPAIR(   0,  -8), SPAIR(  -6, -16), SPAIR( -12, -21)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 10, 32);
const scorepair_t RookOnOpenFile     = SPAIR( 33, 12);
const scorepair_t RookOnBlockedFile  = SPAIR( -6,-14);
const scorepair_t RookXrayQueen      = SPAIR( 11,  5);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -58,  20), SPAIR( -42, -34), SPAIR( -34,  25), SPAIR( -28,  53),
    SPAIR( -21,  71), SPAIR( -17,  89), SPAIR( -11,  98), SPAIR(  -8, 103),
    SPAIR(  -5,  96)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -47, -30), SPAIR( -43, -45), SPAIR( -28, -16), SPAIR( -24,  16),
    SPAIR( -17,  34), SPAIR( -12,  48), SPAIR(  -8,  56), SPAIR(  -8,  57),
    SPAIR( -10,  60), SPAIR(  -6,  57), SPAIR(  -3,  51), SPAIR(  -0,  46),
    SPAIR(   3,  39), SPAIR(  10,  30)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-104,   8), SPAIR( -46,  27), SPAIR( -36,  70), SPAIR( -38,  87),
    SPAIR( -32, 100), SPAIR( -29, 111), SPAIR( -27, 123), SPAIR( -20, 126),
    SPAIR( -18, 136), SPAIR( -10, 140), SPAIR(  -8, 145), SPAIR(  -6, 147),
    SPAIR(   2, 146), SPAIR(   5, 142), SPAIR(  13, 136)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -77,-166), SPAIR(  17, 155), SPAIR(  -2, 166), SPAIR(  -6, 144),
    SPAIR(  -2,  94), SPAIR(  -4, 118), SPAIR(  -5, 144), SPAIR(  -4, 167),
    SPAIR(   0, 182), SPAIR(   4, 194), SPAIR(   7, 204), SPAIR(  11, 207),
    SPAIR(  12, 213), SPAIR(  13, 216), SPAIR(  14, 217), SPAIR(  15, 222),
    SPAIR(  16, 221), SPAIR(  17, 217), SPAIR(  22, 210), SPAIR(  30, 202),
    SPAIR(  34, 195), SPAIR(  32, 187), SPAIR(  25, 183), SPAIR(  20, 172),
    SPAIR(  42, 153), SPAIR(  -7, 168), SPAIR(  14, 180), SPAIR(  47, 158)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 42, 73);
const scorepair_t PawnAttacksRook   = SPAIR( 55, 52);
const scorepair_t PawnAttacksQueen  = SPAIR( 48, 10);
const scorepair_t MinorAttacksRook  = SPAIR( 55, 42);
const scorepair_t MinorAttacksQueen = SPAIR( 40, 46);
const scorepair_t RookAttacksQueen  = SPAIR( 49, 37);
