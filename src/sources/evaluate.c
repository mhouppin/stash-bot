...
const scorepair_t Initiative = SPAIR(12, 8);
...
const scorepair_t KnightWeight    = SPAIR(  40,   9);
const scorepair_t BishopWeight    = SPAIR(  25,  17);
const scorepair_t RookWeight      = SPAIR(  19, -41);
const scorepair_t QueenWeight     = SPAIR(  23,   3);
const scorepair_t AttackWeight    = SPAIR(  17, -42);
const scorepair_t WeakKingZone    = SPAIR(  20, -30);
const scorepair_t SafeKnightCheck = SPAIR(  52,   4);
const scorepair_t SafeBishopCheck = SPAIR(  41,  17);
const scorepair_t SafeRookCheck   = SPAIR(  66,  22);
const scorepair_t SafeQueenCheck  = SPAIR(  43,  25);
const scorepair_t QueenlessAttack = SPAIR( -99,  -4);
const scorepair_t SafetyOffset    = SPAIR(  29,  22);

const scorepair_t KingStorm[24] = {
    // Side
    SPAIR(  -8,   0), SPAIR(  -7,   0), SPAIR(  20,   2), SPAIR(   3,   2),
    SPAIR( -18,   7), SPAIR( -21,   3), SPAIR( -42,   4), SPAIR( -13,   0),

    // Front
    SPAIR(   0,   0), SPAIR(  -1,  -1), SPAIR(  30,   7), SPAIR(   5,  -1),
    SPAIR(  -3,   0), SPAIR(  -4,   9), SPAIR(   5,  12), SPAIR(  16,  -2),

    // Center
    SPAIR(   5,   0), SPAIR(  -1,   0), SPAIR(  29,   3), SPAIR(  29,  -2),
    SPAIR(   1,   2), SPAIR( -10,   9), SPAIR(  -8,  22), SPAIR(   5,  -8)
};

const scorepair_t KingShelter[24] = {
    // Side
    SPAIR( -32,   4), SPAIR( -29,  27), SPAIR( -27,  -4), SPAIR( -18,   3),
    SPAIR(  10,   0), SPAIR(   7,   0), SPAIR(   5,   0), SPAIR(   1, -15),

    // Front
    SPAIR(   0,   0), SPAIR(  -7,   4), SPAIR(  -6,  20), SPAIR(   8,   9),
    SPAIR(  11,   3), SPAIR(  21,   0), SPAIR(   4,   0), SPAIR(  21, -13),

    // Center
    SPAIR( -33,  -5), SPAIR(   5, -21), SPAIR(   8,  14), SPAIR(   9,  23),
    SPAIR(  18,   6), SPAIR(  22,   0), SPAIR(   4,   0), SPAIR(  20,   6)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  4, 18);
const scorepair_t KnightOutpost       = SPAIR(  8,-19);
const scorepair_t KnightCenterOutpost = SPAIR( 14,  8);
const scorepair_t KnightSolidOutpost  = SPAIR( 14, 22);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(  -2, -24), SPAIR(   4,  -7), SPAIR(   5,   7), SPAIR(   7,  17),
    SPAIR(   6,  34)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 23, 93);
const scorepair_t BishopShielded     = SPAIR(  6, 12);
const scorepair_t BishopLongDiagonal = SPAIR( 11, 31);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(  14,  25), SPAIR(  14,  17), SPAIR(  11,   8), SPAIR(   6,   1),
    SPAIR(   0,  -6), SPAIR(  -7, -12), SPAIR( -14, -17)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 11, 27);
const scorepair_t RookOnOpenFile     = SPAIR( 30, 12);
const scorepair_t RookOnBlockedFile  = SPAIR( -6,-11);
const scorepair_t RookXrayQueen      = SPAIR( 10,  3);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -53,  19), SPAIR( -40, -35), SPAIR( -34,  26), SPAIR( -28,  55),
    SPAIR( -22,  70), SPAIR( -18,  88), SPAIR( -13,  96), SPAIR(  -9, 102),
    SPAIR(  -9,  98)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -41, -34), SPAIR( -41, -45), SPAIR( -27, -12), SPAIR( -24,  17),
    SPAIR( -17,  34), SPAIR( -13,  48), SPAIR( -10,  55), SPAIR( -11,  58),
    SPAIR( -10,  58), SPAIR(  -9,  55), SPAIR(  -7,  49), SPAIR(  -2,  46),
    SPAIR(   3,  39), SPAIR(   9,  30)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-102,   9), SPAIR( -43,  29), SPAIR( -34,  72), SPAIR( -37,  89),
    SPAIR( -33, 101), SPAIR( -30, 111), SPAIR( -28, 123), SPAIR( -21, 126),
    SPAIR( -18, 133), SPAIR( -12, 138), SPAIR( -10, 143), SPAIR(  -6, 144),
    SPAIR(   0, 143), SPAIR(   4, 140), SPAIR(  12, 133)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -76,-165), SPAIR(  18, 157), SPAIR(   0, 165), SPAIR(  -5, 142),
    SPAIR(   0,  93), SPAIR(  -4, 118), SPAIR(  -3, 146), SPAIR(  -2, 169),
    SPAIR(   1, 183), SPAIR(   3, 198), SPAIR(   6, 204), SPAIR(  10, 208),
    SPAIR(  11, 212), SPAIR(  13, 215), SPAIR(  14, 218), SPAIR(  12, 221),
    SPAIR(  15, 220), SPAIR(  15, 218), SPAIR(  22, 210), SPAIR(  27, 201),
    SPAIR(  33, 194), SPAIR(  33, 187), SPAIR(  25, 182), SPAIR(  20, 172),
    SPAIR(  42, 153), SPAIR(  -7, 167), SPAIR(  14, 180), SPAIR(  47, 158)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 37, 70);
const scorepair_t PawnAttacksRook   = SPAIR( 52, 50);
const scorepair_t PawnAttacksQueen  = SPAIR( 42,  9);
const scorepair_t MinorAttacksRook  = SPAIR( 50, 39);
const scorepair_t MinorAttacksQueen = SPAIR( 34, 46);
const scorepair_t RookAttacksQueen  = SPAIR( 45, 36);
