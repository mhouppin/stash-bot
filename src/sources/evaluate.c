...
// Special eval terms
const scorepair_t CastlingBonus = SPAIR(99, -53);
const scorepair_t Initiative = SPAIR(25, 25);

// King Safety eval terms
const scorepair_t KnightWeight    = SPAIR(  47,  16);
const scorepair_t BishopWeight    = SPAIR(  36,  28);
const scorepair_t RookWeight      = SPAIR(  37, -34);
const scorepair_t QueenWeight     = SPAIR(   7,   7);
const scorepair_t AttackWeight    = SPAIR(   9,   7);
const scorepair_t WeakKingZone    = SPAIR(  28, -22);
const scorepair_t SafeKnightCheck = SPAIR(  70,   7);
const scorepair_t SafeBishopCheck = SPAIR(  36,  30);
const scorepair_t SafeRookCheck   = SPAIR(  85,  40);
const scorepair_t SafeQueenCheck  = SPAIR(  52,  50);
const scorepair_t UnsafeCheck     = SPAIR(  19,  32);
const scorepair_t QueenlessAttack = SPAIR( -85,  -6);
const scorepair_t SafetyOffset    = SPAIR(  21,  35);
...
const scorepair_t KingStorm[24] = {
    SPAIR( -11,  -2), SPAIR( -12,  -1), SPAIR(  18,   4), SPAIR(  -5,   4),
    SPAIR( -29,  11), SPAIR( -18,   3), SPAIR( -45,   3), SPAIR( -17,   1),
    SPAIR(   0,   0), SPAIR( -17,  -5), SPAIR(  40,  10), SPAIR(   3,  -0),
    SPAIR(  -5,   2), SPAIR(  -5,  11), SPAIR(  10,  18), SPAIR(  14,  -1),
    SPAIR(  -1,  -1), SPAIR(  -2,   0), SPAIR(  39,   3), SPAIR(  26,  -3),
    SPAIR(  -7,   2), SPAIR( -12,  15), SPAIR(  -6,  34), SPAIR(   3, -12)
};

const scorepair_t KingShelter[24] = {
    SPAIR( -42,   7), SPAIR( -34,  40), SPAIR( -32, -10), SPAIR( -24,   5),
    SPAIR(  10,  -1), SPAIR(   6,  -0), SPAIR(   5,  -0), SPAIR( -10, -22),
    SPAIR(   0,   0), SPAIR(  -6,   0), SPAIR(  -2,  31), SPAIR(   1,  14),
    SPAIR(   9,   6), SPAIR(  20,   0), SPAIR(   7,   0), SPAIR(  16, -18),
    SPAIR( -39,  -9), SPAIR(   9, -35), SPAIR(   5,  22), SPAIR(  11,  38),
    SPAIR(  10,   8), SPAIR(  24,   0), SPAIR(   6,  -0), SPAIR(  18,  11)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  4, 15);
const scorepair_t KnightOutpost       = SPAIR( 13,-19);
const scorepair_t KnightCenterOutpost = SPAIR( 14, 10);
const scorepair_t KnightSolidOutpost  = SPAIR( 22, 25);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(   6, -19), SPAIR(   9,   1), SPAIR(  10,  18), SPAIR(  12,  31),
    SPAIR(   9,  46)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 22,103);
const scorepair_t BishopShielded     = SPAIR(  2,  7);
const scorepair_t BishopLongDiagonal = SPAIR( 14, 39);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(  15,  37), SPAIR(  18,  31), SPAIR(  16,  17), SPAIR(  10,  10),
    SPAIR(   4,  -0), SPAIR(   1,  -7), SPAIR(  -6, -16)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 16, 29);
const scorepair_t RookOnOpenFile     = SPAIR( 32, 13);
const scorepair_t RookOnBlockedFile  = SPAIR( -5,-17);
const scorepair_t RookXrayQueen      = SPAIR( 16,  6);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -62,  14), SPAIR( -45, -38), SPAIR( -33,  29), SPAIR( -24,  58),
    SPAIR( -16,  78), SPAIR( -13, 100), SPAIR(  -6, 107), SPAIR(   1, 114),
    SPAIR(   1, 111)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -58, -42), SPAIR( -41, -42), SPAIR( -26, -13), SPAIR( -24,  20),
    SPAIR( -16,  41), SPAIR(  -8,  51), SPAIR(  -3,  61), SPAIR(  -7,  64),
    SPAIR(  -8,  64), SPAIR(  -1,  63), SPAIR(  -3,  58), SPAIR(   8,  51),
    SPAIR(   6,  42), SPAIR(  13,  35)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-112,  -4), SPAIR( -49,  19), SPAIR( -32,  63), SPAIR( -33,  86),
    SPAIR( -28, 108), SPAIR( -30, 125), SPAIR( -30, 133), SPAIR( -25, 135),
    SPAIR( -20, 143), SPAIR(  -9, 150), SPAIR(  -8, 154), SPAIR(   0, 161),
    SPAIR(  13, 161), SPAIR(  16, 157), SPAIR(  29, 154)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -68,-156), SPAIR(  27, 165), SPAIR(   7, 160), SPAIR(   5, 138),
    SPAIR(   4,  89), SPAIR(   6, 120), SPAIR(   5, 146), SPAIR(   5, 173),
    SPAIR(   8, 192), SPAIR(  11, 203), SPAIR(   8, 218), SPAIR(  11, 224),
    SPAIR(   9, 226), SPAIR(   9, 233), SPAIR(  12, 234), SPAIR(   8, 238),
    SPAIR(  11, 236), SPAIR(  15, 234), SPAIR(  20, 224), SPAIR(  32, 215),
    SPAIR(  37, 208), SPAIR(  39, 198), SPAIR(  34, 191), SPAIR(  26, 177),
    SPAIR(  43, 154), SPAIR(  -7, 167), SPAIR(  14, 178), SPAIR(  46, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 64, 85);
const scorepair_t PawnAttacksRook   = SPAIR( 62, 60);
const scorepair_t PawnAttacksQueen  = SPAIR( 54, 11);
const scorepair_t MinorAttacksRook  = SPAIR( 65, 52);
const scorepair_t MinorAttacksQueen = SPAIR( 48, 55);
const scorepair_t RookAttacksQueen  = SPAIR( 55, 40);
