...
// Special eval terms
const scorepair_t CastlingBonus = SPAIR(88, -54);
const scorepair_t Initiative = SPAIR(29, 37);

// King Safety eval terms
const scorepair_t KnightWeight    = SPAIR(  39,  16);
const scorepair_t BishopWeight    = SPAIR(  30,  29);
const scorepair_t RookWeight      = SPAIR(  32, -34);
const scorepair_t QueenWeight     = SPAIR(   7,   7);
const scorepair_t AttackWeight    = SPAIR(  11,   9);
const scorepair_t WeakKingZone    = SPAIR(  35, -21);
const scorepair_t SafeKnightCheck = SPAIR(  89,   8);
const scorepair_t SafeBishopCheck = SPAIR(  38,  30);
const scorepair_t SafeRookCheck   = SPAIR( 104,  40);
const scorepair_t SafeQueenCheck  = SPAIR(  68,  51);
const scorepair_t UnsafeCheck     = SPAIR(  15,  32);
const scorepair_t QueenlessAttack = SPAIR( -79,  -6);
const scorepair_t SafetyOffset    = SPAIR(  30,  35);
...
const scorepair_t KingStorm[24] = {
    SPAIR( -12,  -2), SPAIR( -13,  -1), SPAIR(  25,   4), SPAIR(  -3,   4),
    SPAIR( -21,  11), SPAIR( -32,   3), SPAIR( -42,   3), SPAIR( -15,   1),
    SPAIR(   0,   0), SPAIR( -18,  -5), SPAIR(  45,  10), SPAIR(   4,  -0),
    SPAIR(  -6,   2), SPAIR(  -3,  11), SPAIR(  10,  18), SPAIR(  17,  -1),
    SPAIR(  -1,  -1), SPAIR(  -2,   0), SPAIR(  39,   3), SPAIR(  17,  -3),
    SPAIR(  -3,   2), SPAIR(  -9,  15), SPAIR(   5,  34), SPAIR(   4, -12)
};

const scorepair_t KingShelter[24] = {
    SPAIR( -49,   7), SPAIR( -24,  41), SPAIR( -33,  -9), SPAIR( -21,   5),
    SPAIR(  12,  -1), SPAIR(   5,  -0), SPAIR(   5,  -0), SPAIR( -11, -21),
    SPAIR(   0,   0), SPAIR(   3,   1), SPAIR(  -2,  31), SPAIR(  11,  14),
    SPAIR(   7,   6), SPAIR(  19,  -0), SPAIR(   8,   0), SPAIR(   8, -17),
    SPAIR( -35,  -8), SPAIR(  19, -35), SPAIR(  -1,  22), SPAIR(  13,  37),
    SPAIR(  14,   8), SPAIR(  23,  -0), SPAIR(   7,   0), SPAIR(  15,  11)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  0, 20);
const scorepair_t KnightOutpost       = SPAIR( 14,-17);
const scorepair_t KnightCenterOutpost = SPAIR( 13, 17);
const scorepair_t KnightSolidOutpost  = SPAIR( 25, 31);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(   5, -17), SPAIR(   5,   1), SPAIR(   6,  15), SPAIR(   9,  25),
    SPAIR(  12,  52)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 19,114);
const scorepair_t BishopShielded     = SPAIR( -2, -2);
const scorepair_t BishopLongDiagonal = SPAIR( 13, 39);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(   9,  35), SPAIR(  16,  31), SPAIR(  15,  21), SPAIR(   8,   5),
    SPAIR(   3,   2), SPAIR(   2,  -9), SPAIR(  -9, -19)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 21, 25);
const scorepair_t RookOnOpenFile     = SPAIR( 50, 30);
const scorepair_t RookOnBlockedFile  = SPAIR( -7,-23);
const scorepair_t RookXrayQueen      = SPAIR( 18,  7);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -63,  13), SPAIR( -54, -38), SPAIR( -38,  30), SPAIR( -25,  51),
    SPAIR( -22,  77), SPAIR( -12, 100), SPAIR(  -5, 114), SPAIR(   5, 117),
    SPAIR(   8, 109)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -78, -49), SPAIR( -44, -42), SPAIR( -22, -17), SPAIR( -21,  20),
    SPAIR( -11,  35), SPAIR(  -7,  59), SPAIR(  -7,  60), SPAIR(  -9,  67),
    SPAIR(  -7,  68), SPAIR(  -4,  63), SPAIR(   1,  58), SPAIR(   9,  51),
    SPAIR(   6,  42), SPAIR(  13,  35)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-111,  -6), SPAIR( -55,  19), SPAIR( -35,  56), SPAIR( -40,  81),
    SPAIR( -31, 109), SPAIR( -27, 124), SPAIR( -34, 133), SPAIR( -29, 131),
    SPAIR( -29, 138), SPAIR( -18, 153), SPAIR(  -9, 158), SPAIR(   8, 164),
    SPAIR(   9, 165), SPAIR(  17, 162), SPAIR(  28, 155)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -68,-156), SPAIR(  22, 164), SPAIR(   0, 160), SPAIR(  -1, 137),
    SPAIR(   2,  88), SPAIR(   6, 117), SPAIR(   3, 144), SPAIR(   7, 169),
    SPAIR(   6, 188), SPAIR(  10, 202), SPAIR(  10, 217), SPAIR(  10, 226),
    SPAIR(   7, 227), SPAIR(   5, 233), SPAIR(  11, 236), SPAIR(  14, 240),
    SPAIR(  14, 237), SPAIR(  17, 235), SPAIR(  23, 226), SPAIR(  32, 216),
    SPAIR(  37, 208), SPAIR(  38, 197), SPAIR(  34, 190), SPAIR(  26, 177),
    SPAIR(  43, 154), SPAIR(  -7, 167), SPAIR(  14, 178), SPAIR(  46, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 75, 87);
const scorepair_t PawnAttacksRook   = SPAIR( 65, 61);
const scorepair_t PawnAttacksQueen  = SPAIR( 54, 11);
const scorepair_t MinorAttacksRook  = SPAIR( 63, 54);
const scorepair_t MinorAttacksQueen = SPAIR( 54, 55);
const scorepair_t RookAttacksQueen  = SPAIR( 60, 41);
