...
// Special eval terms
const scorepair_t CastlingBonus = SPAIR(92, -63);
...
const scorepair_t KnightWeight    = SPAIR(  34,  12);
const scorepair_t BishopWeight    = SPAIR(  21,  20);
const scorepair_t RookWeight      = SPAIR(  16, -37);
const scorepair_t QueenWeight     = SPAIR(  20,   4);
const scorepair_t AttackWeight    = SPAIR(  17, -17);
const scorepair_t WeakKingZone    = SPAIR(  15, -12);
const scorepair_t SafeKnightCheck = SPAIR(  51,   4);
const scorepair_t SafeBishopCheck = SPAIR(  37,  17);
const scorepair_t SafeRookCheck   = SPAIR(  59,  22);
const scorepair_t SafeQueenCheck  = SPAIR(  37,  24);
const scorepair_t QueenlessAttack = SPAIR( -80,  -1);
const scorepair_t SafetyOffset    = SPAIR(  28,  28);
...
const scorepair_t KingStorm[24] = {
    SPAIR(  -4,  -0), SPAIR(  -7,  -0), SPAIR(  15,   2), SPAIR(   0,   3),
    SPAIR( -17,   8), SPAIR( -22,   4), SPAIR( -41,   3), SPAIR( -16,   3),
    SPAIR(   0,   0), SPAIR(  -2,  -1), SPAIR(  29,   7), SPAIR(   7,   0),
    SPAIR(  -2,   2), SPAIR(  -2,  10), SPAIR(   5,  12), SPAIR(  13,   0),
    SPAIR(   7,  -0), SPAIR(  -0,  -0), SPAIR(  29,   3), SPAIR(  27,  -1),
    SPAIR(  -1,   1), SPAIR(  -7,   9), SPAIR(  -7,  22), SPAIR(   2,  -3)
};

const scorepair_t KingShelter[24] = {
    SPAIR( -35,   5), SPAIR( -29,  28), SPAIR( -26,  -4), SPAIR( -21,   3),
    SPAIR(   9,  -0), SPAIR(   8,  -0), SPAIR(   4,  -0), SPAIR(  -1, -13),
    SPAIR(   0,   0), SPAIR(  -3,   3), SPAIR(  -6,  22), SPAIR(   6,   9),
    SPAIR(   9,   3), SPAIR(  19,   0), SPAIR(   4,   0), SPAIR(  21,  -9),
    SPAIR( -33,  -4), SPAIR(   4, -22), SPAIR(   8,  14), SPAIR(  12,  24),
    SPAIR(  15,   5), SPAIR(  24,   0), SPAIR(   5,   0), SPAIR(  18,  10)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  3, 15);
const scorepair_t KnightOutpost       = SPAIR(  6,-21);
const scorepair_t KnightCenterOutpost = SPAIR( 13,  9);
const scorepair_t KnightSolidOutpost  = SPAIR(  9, 17);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(  -8, -36), SPAIR(  -7, -19), SPAIR(  -6,  -6), SPAIR(  -3,   3),
    SPAIR(  -5,  20)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 12, 81);
const scorepair_t BishopShielded     = SPAIR(  3, 10);
const scorepair_t BishopLongDiagonal = SPAIR(  7, 25);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(   6,  12), SPAIR(   5,   4), SPAIR(   2,  -3), SPAIR(  -2,  -9),
    SPAIR(  -7, -15), SPAIR( -12, -20), SPAIR( -17, -23)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR(  8, 22);
const scorepair_t RookOnOpenFile     = SPAIR( 25,  5);
const scorepair_t RookOnBlockedFile  = SPAIR( -4,-13);
const scorepair_t RookXrayQueen      = SPAIR(  7, 11);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -53,  20), SPAIR( -42, -26), SPAIR( -37,  27), SPAIR( -33,  48),
    SPAIR( -28,  61), SPAIR( -25,  76), SPAIR( -21,  82), SPAIR( -18,  87),
    SPAIR( -16,  82)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -44, -24), SPAIR( -41, -31), SPAIR( -31,  -8), SPAIR( -28,  15),
    SPAIR( -23,  29), SPAIR( -19,  38), SPAIR( -16,  44), SPAIR( -16,  45),
    SPAIR( -17,  46), SPAIR( -14,  43), SPAIR( -10,  39), SPAIR(  -3,  33),
    SPAIR(   2,  34), SPAIR(  10,  25)
};

const scorepair_t MobilityR[15] = {
    SPAIR( -95,  14), SPAIR( -47,  38), SPAIR( -40,  73), SPAIR( -41,  84),
    SPAIR( -37,  94), SPAIR( -35, 101), SPAIR( -33, 111), SPAIR( -28, 113),
    SPAIR( -26, 121), SPAIR( -21, 124), SPAIR( -18, 127), SPAIR( -17, 129),
    SPAIR( -10, 129), SPAIR(  -5, 126), SPAIR(  13, 118)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -74,-163), SPAIR(  12, 158), SPAIR(  -2, 163), SPAIR(  -6, 142),
    SPAIR(  -3,  96), SPAIR(  -5, 124), SPAIR(  -6, 147), SPAIR(  -6, 167),
    SPAIR(  -3, 178), SPAIR(  -1, 185), SPAIR(   1, 192), SPAIR(   4, 193),
    SPAIR(   5, 198), SPAIR(   5, 200), SPAIR(   6, 200), SPAIR(   6, 204),
    SPAIR(   8, 201), SPAIR(   9, 197), SPAIR(  13, 193), SPAIR(  23, 186),
    SPAIR(  26, 180), SPAIR(  28, 176), SPAIR(  23, 177), SPAIR(  19, 166),
    SPAIR(  41, 150), SPAIR(  -8, 165), SPAIR(  14, 179), SPAIR(  46, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 30, 63);
const scorepair_t PawnAttacksRook   = SPAIR( 48, 46);
const scorepair_t PawnAttacksQueen  = SPAIR( 37,  9);
const scorepair_t MinorAttacksRook  = SPAIR( 40, 36);
const scorepair_t MinorAttacksQueen = SPAIR( 26, 47);
const scorepair_t RookAttacksQueen  = SPAIR( 33, 37);
