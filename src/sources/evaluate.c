...
// Special eval terms
const scorepair_t CastlingBonus = SPAIR(103, -54);
const scorepair_t Initiative = SPAIR(18, 9);

// King Safety eval terms
const scorepair_t KnightWeight    = SPAIR(  50,  18);
const scorepair_t BishopWeight    = SPAIR(  33,  31);
const scorepair_t RookWeight      = SPAIR(  31, -35);
const scorepair_t QueenWeight     = SPAIR(  11,   7);
const scorepair_t AttackWeight    = SPAIR(   9,  12);
const scorepair_t WeakKingZone    = SPAIR(  21, -25);
const scorepair_t SafeKnightCheck = SPAIR(  64,   8);
const scorepair_t SafeBishopCheck = SPAIR(  30,  33);
const scorepair_t SafeRookCheck   = SPAIR(  76,  44);
const scorepair_t SafeQueenCheck  = SPAIR(  45,  56);
const scorepair_t UnsafeCheck     = SPAIR(  18,  35);
const scorepair_t QueenlessAttack = SPAIR( -90,  -8);
const scorepair_t SafetyOffset    = SPAIR(  31,  36);
...
const scorepair_t KingStorm[24] = {
    SPAIR( -11,  -2), SPAIR( -13,  -1), SPAIR(  21,   5), SPAIR(   1,   4),
    SPAIR( -20,  12), SPAIR( -24,   3), SPAIR( -46,   3), SPAIR( -15,  -0),
    SPAIR(   0,   0), SPAIR( -20,  -5), SPAIR(  39,  11), SPAIR(   6,  -0),
    SPAIR(   0,   2), SPAIR(  -2,  12), SPAIR(   8,  20), SPAIR(  18,  -2),
    SPAIR(  -0,  -1), SPAIR(  -2,  -0), SPAIR(  38,   3), SPAIR(  30,  -4),
    SPAIR(  -1,   2), SPAIR( -10,  17), SPAIR(  -8,  38), SPAIR(   4, -16)
};

const scorepair_t KingShelter[24] = {
    SPAIR( -42,   7), SPAIR( -34,  44), SPAIR( -33, -11), SPAIR( -20,   5),
    SPAIR(  10,  -1), SPAIR(   7,  -0), SPAIR(   5,  -0), SPAIR(  -2, -24),
    SPAIR(   0,   0), SPAIR(  -5,  -1), SPAIR(  -3,  34), SPAIR(   5,  16),
    SPAIR(  10,   7), SPAIR(  20,  -0), SPAIR(   7,  -0), SPAIR(  19, -19),
    SPAIR( -36, -10), SPAIR(   4, -39), SPAIR(   7,  24), SPAIR(  17,  42),
    SPAIR(  14,   9), SPAIR(  23,   0), SPAIR(   6,  -0), SPAIR(  19,  12)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  6, 20);
const scorepair_t KnightOutpost       = SPAIR( 10,-22);
const scorepair_t KnightCenterOutpost = SPAIR( 20, 11);
const scorepair_t KnightSolidOutpost  = SPAIR( 16, 23);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(   5, -17), SPAIR(  10,   5), SPAIR(  11,  21), SPAIR(  15,  31),
    SPAIR(  13,  53)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 26,111);
const scorepair_t BishopShielded     = SPAIR(  8, 13);
const scorepair_t BishopLongDiagonal = SPAIR( 14, 37);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(  21,  46), SPAIR(  22,  33), SPAIR(  17,  21), SPAIR(  12,  11),
    SPAIR(   5,   1), SPAIR(  -1,  -8), SPAIR(  -8, -15)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR(  9, 35);
const scorepair_t RookOnOpenFile     = SPAIR( 34, 11);
const scorepair_t RookOnBlockedFile  = SPAIR( -8,-16);
const scorepair_t RookXrayQueen      = SPAIR( 14,  6);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -57,  17), SPAIR( -42, -41), SPAIR( -33,  28), SPAIR( -25,  61),
    SPAIR( -17,  81), SPAIR( -12, 102), SPAIR(  -6, 112), SPAIR(  -1, 119),
    SPAIR(   2, 111)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -47, -39), SPAIR( -44, -48), SPAIR( -27, -16), SPAIR( -24,  20),
    SPAIR( -15,  41), SPAIR( -10,  56), SPAIR(  -5,  64), SPAIR(  -5,  67),
    SPAIR(  -6,  68), SPAIR(  -3,  66), SPAIR(  -1,  59), SPAIR(   9,  53),
    SPAIR(   7,  45), SPAIR(  14,  36)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-111,  -6), SPAIR( -44,  19), SPAIR( -31,  60), SPAIR( -36,  86),
    SPAIR( -31, 106), SPAIR( -32, 125), SPAIR( -30, 136), SPAIR( -23, 139),
    SPAIR( -17, 150), SPAIR(  -6, 156), SPAIR(  -4, 162), SPAIR(   4, 165),
    SPAIR(  12, 165), SPAIR(  18, 163), SPAIR(  33, 154)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -66,-153), SPAIR(  28, 167), SPAIR(   9, 160), SPAIR(   1, 136),
    SPAIR(   7,  88), SPAIR(   2, 119), SPAIR(   2, 145), SPAIR(   3, 172),
    SPAIR(   3, 192), SPAIR(   7, 205), SPAIR(   8, 220), SPAIR(  11, 224),
    SPAIR(  13, 231), SPAIR(  13, 236), SPAIR(  16, 238), SPAIR(  15, 245),
    SPAIR(  18, 243), SPAIR(  19, 238), SPAIR(  24, 231), SPAIR(  34, 220),
    SPAIR(  38, 212), SPAIR(  41, 201), SPAIR(  36, 192), SPAIR(  28, 179),
    SPAIR(  43, 155), SPAIR(  -7, 167), SPAIR(  14, 178), SPAIR(  46, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 49, 85);
const scorepair_t PawnAttacksRook   = SPAIR( 64, 60);
const scorepair_t PawnAttacksQueen  = SPAIR( 52, 11);
const scorepair_t MinorAttacksRook  = SPAIR( 64, 49);
const scorepair_t MinorAttacksQueen = SPAIR( 45, 56);
