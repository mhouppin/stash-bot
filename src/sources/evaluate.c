...
// Special eval terms
const scorepair_t CastlingBonus = SPAIR(84, -49);
const scorepair_t Initiative = SPAIR(30, 35);

// King Safety eval terms
const scorepair_t KnightWeight    = SPAIR(  43,  16);
const scorepair_t BishopWeight    = SPAIR(  26,  29);
const scorepair_t RookWeight      = SPAIR(  33, -34);
const scorepair_t QueenWeight     = SPAIR(   8,   7);
const scorepair_t AttackWeight    = SPAIR(  10,   8);
const scorepair_t WeakKingZone    = SPAIR(  34, -21);
const scorepair_t SafeKnightCheck = SPAIR(  89,   8);
const scorepair_t SafeBishopCheck = SPAIR(  43,  30);
const scorepair_t SafeRookCheck   = SPAIR( 101,  40);
const scorepair_t SafeQueenCheck  = SPAIR(  61,  50);
const scorepair_t UnsafeCheck     = SPAIR(  16,  32);
const scorepair_t QueenlessAttack = SPAIR( -82,  -6);
const scorepair_t SafetyOffset    = SPAIR(  31,  35);
...
const scorepair_t KingStorm[24] = {
    SPAIR( -12,  -2), SPAIR( -12,  -1), SPAIR(  24,   4), SPAIR(  -3,   4),
    SPAIR( -33,  11), SPAIR( -20,   3), SPAIR( -43,   2), SPAIR( -18,   1),
    SPAIR(   0,   0), SPAIR( -15,  -5), SPAIR(  40,  10), SPAIR(   6,   0),
    SPAIR(  -7,   2), SPAIR(  -5,  10), SPAIR(  13,  18), SPAIR(  20,  -1),
    SPAIR(   0,  -1), SPAIR(  -2,  -0), SPAIR(  39,   3), SPAIR(  22,  -3),
    SPAIR(  -7,   2), SPAIR( -13,  15), SPAIR(   5,  34), SPAIR(   9, -12)
};

const scorepair_t KingShelter[24] = {
    SPAIR( -46,   7), SPAIR( -29,  41), SPAIR( -36, -10), SPAIR( -23,   5),
    SPAIR(  14,  -1), SPAIR(   4,  -0), SPAIR(   5,  -0), SPAIR(  -9, -21),
    SPAIR(   0,   0), SPAIR(  -4,   0), SPAIR(  -9,  31), SPAIR(   7,  14),
    SPAIR(  13,   6), SPAIR(  23,   0), SPAIR(   7,   0), SPAIR(  18, -17),
    SPAIR( -29,  -8), SPAIR(  21, -35), SPAIR(  -5,  22), SPAIR(   5,  37),
    SPAIR(  13,   8), SPAIR(  24,   0), SPAIR(   6,  -0), SPAIR(  19,  11)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR( -1, 24);
const scorepair_t KnightOutpost       = SPAIR( 17,-14);
const scorepair_t KnightCenterOutpost = SPAIR( 14, 13);
const scorepair_t KnightSolidOutpost  = SPAIR( 21, 29);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(   3, -22), SPAIR(   4,  -7), SPAIR(   8,  21), SPAIR(  11,  28),
    SPAIR(  12,  58)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR(  7,106);
const scorepair_t BishopShielded     = SPAIR(  1,  7);
const scorepair_t BishopLongDiagonal = SPAIR( 17, 36);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(   5,  32), SPAIR(  15,  28), SPAIR(  12,  17), SPAIR(   8,  14),
    SPAIR(   6,   5), SPAIR(   4, -10), SPAIR(   2, -14)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 15, 28);
const scorepair_t RookOnOpenFile     = SPAIR( 40, 30);
const scorepair_t RookOnBlockedFile  = SPAIR( -6,-17);
const scorepair_t RookXrayQueen      = SPAIR( 18,  5);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -63,  11), SPAIR( -46, -38), SPAIR( -34,  22), SPAIR( -22,  56),
    SPAIR( -17,  74), SPAIR( -14, 107), SPAIR(  -9, 112), SPAIR(  -0, 121),
    SPAIR(   1, 108)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -69, -48), SPAIR( -44, -47), SPAIR( -25, -16), SPAIR( -21,  15),
    SPAIR( -10,  38), SPAIR( -11,  54), SPAIR(  -6,  68), SPAIR(  -4,  69),
    SPAIR(  -6,  70), SPAIR(   1,  63), SPAIR(  -5,  57), SPAIR(   6,  52),
    SPAIR(   6,  43), SPAIR(  13,  35)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-112,  -4), SPAIR( -62,  12), SPAIR( -40,  56), SPAIR( -39,  84),
    SPAIR( -31, 109), SPAIR( -28, 119), SPAIR( -30, 133), SPAIR( -23, 136),
    SPAIR( -23, 145), SPAIR( -10, 151), SPAIR( -11, 160), SPAIR(  -2, 161),
    SPAIR(  10, 163), SPAIR(  16, 163), SPAIR(  28, 156)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -68,-156), SPAIR(  23, 165), SPAIR(  10, 160), SPAIR(   4, 137),
    SPAIR(   9,  87), SPAIR(   4, 119), SPAIR(   5, 143), SPAIR(   7, 169),
    SPAIR(   4, 190), SPAIR(   8, 201), SPAIR(   3, 218), SPAIR(   7, 224),
    SPAIR(   9, 227), SPAIR(   9, 234), SPAIR(  13, 234), SPAIR(   9, 239),
    SPAIR(  10, 239), SPAIR(  16, 234), SPAIR(  20, 226), SPAIR(  31, 216),
    SPAIR(  37, 208), SPAIR(  38, 198), SPAIR(  34, 190), SPAIR(  26, 177),
    SPAIR(  43, 154), SPAIR(  -7, 167), SPAIR(  14, 178), SPAIR(  46, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 77, 88);
const scorepair_t PawnAttacksRook   = SPAIR( 65, 59);
const scorepair_t PawnAttacksQueen  = SPAIR( 57, 11);
const scorepair_t MinorAttacksRook  = SPAIR( 62, 52);
const scorepair_t MinorAttacksQueen = SPAIR( 58, 56);
const scorepair_t RookAttacksQueen  = SPAIR( 59, 41);
