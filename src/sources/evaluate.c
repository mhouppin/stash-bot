...
const scorepair_t KnightWeight    = SPAIR(  39,  13);
const scorepair_t BishopWeight    = SPAIR(  22,  23);
...
const scorepair_t SafeKnightCheck = SPAIR(  55,   7);
const scorepair_t SafeBishopCheck = SPAIR(  47,  24);
const scorepair_t SafeRookCheck   = SPAIR(  65,  33);
const scorepair_t SafeQueenCheck  = SPAIR(  44,  34);
const scorepair_t QueenlessAttack = SPAIR( -98,   3);
const scorepair_t SafetyOffset    = SPAIR(  34,  39);

const scorepair_t KingStorm[24] = {
    // Side
    SPAIR(  -6,  -1), SPAIR(  -9,  -1), SPAIR(  21,   4), SPAIR(   1,   4),
    SPAIR( -20,   9), SPAIR( -23,   4), SPAIR( -45,   4), SPAIR( -17,   6),

    // Front
    SPAIR(   0,   0), SPAIR(  -2,  -2), SPAIR(  32,  10), SPAIR(   6,   1),
    SPAIR(  -2,   2), SPAIR(  -3,  10), SPAIR(   7,  16), SPAIR(  16,   3),

    // Center
    SPAIR(   5,  -1), SPAIR(  -1,   0), SPAIR(  34,   4), SPAIR(  27,  -2),
    SPAIR(  -1,   4), SPAIR(  -8,  12), SPAIR(  -6,  27), SPAIR(   4,  -2)
};

const scorepair_t KingShelter[24] = {
    // Side
    SPAIR( -32,   8), SPAIR( -29,  35), SPAIR( -30,  -5), SPAIR( -21,   5),
    SPAIR(   8,  -1), SPAIR(   4,   0), SPAIR(   5,   0), SPAIR(  -3, -16),

    // Front
    SPAIR(   0,   0), SPAIR(  -4,   7), SPAIR(  -5,  27), SPAIR(   8,  13),
    SPAIR(  11,   4), SPAIR(  19,   0), SPAIR(   5,   0), SPAIR(  22, -12),

    // Center
    SPAIR( -33,  -5), SPAIR(   3, -25), SPAIR(  10,  19), SPAIR(  12,  32),
    SPAIR(  18,   7), SPAIR(  22,   0), SPAIR(   6,   0), SPAIR(  19,  12)
};
...
const scorepair_t ClosedPosKnight[5] = {
    SPAIR(  -3, -24), SPAIR(   5,  -4), SPAIR(   5,  11), SPAIR(   8,  21),
    SPAIR(   7,  38)
};
...
const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(  14,  31), SPAIR(  14,  21), SPAIR(  11,  11), SPAIR(   6,   4),
    SPAIR(   0,  -5), SPAIR(  -6, -13), SPAIR( -12, -18)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 12, 30);
const scorepair_t RookOnOpenFile     = SPAIR( 37, 11);
const scorepair_t RookOnBlockedFile  = SPAIR( -6,-14);
const scorepair_t RookXrayQueen      = SPAIR( 11,  4);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -58,  22), SPAIR( -43, -36), SPAIR( -36,  28), SPAIR( -29,  57),
    SPAIR( -21,  72), SPAIR( -16,  90), SPAIR( -11,  99), SPAIR(  -6, 105),
    SPAIR(  -4,  99)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -46, -32), SPAIR( -44, -47), SPAIR( -29, -14), SPAIR( -25,  17),
    SPAIR( -17,  36), SPAIR( -12,  50), SPAIR(  -8,  57), SPAIR(  -8,  60),
    SPAIR(  -8,  60), SPAIR(  -7,  57), SPAIR(  -5,  51), SPAIR(  -1,  48),
    SPAIR(   4,  39), SPAIR(   9,  31)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-103,   5), SPAIR( -47,  26), SPAIR( -36,  71), SPAIR( -39,  89),
    SPAIR( -33, 101), SPAIR( -30, 114), SPAIR( -28, 127), SPAIR( -21, 131),
    SPAIR( -17, 138), SPAIR( -10, 143), SPAIR(  -7, 147), SPAIR(  -4, 150),
    SPAIR(   2, 149), SPAIR(   7, 144), SPAIR(  13, 138)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -71,-159), SPAIR(  18, 164), SPAIR(  -3, 165), SPAIR(  -7, 140),
    SPAIR(  -2,  92), SPAIR(  -5, 119), SPAIR(  -4, 146), SPAIR(  -3, 169),
    SPAIR(   1, 185), SPAIR(   4, 199), SPAIR(   7, 208), SPAIR(  11, 211),
    SPAIR(  13, 216), SPAIR(  14, 218), SPAIR(  16, 222), SPAIR(  15, 227),
    SPAIR(  16, 227), SPAIR(  18, 223), SPAIR(  23, 215), SPAIR(  31, 204),
    SPAIR(  36, 199), SPAIR(  34, 190), SPAIR(  28, 183), SPAIR(  22, 173),
    SPAIR(  42, 154), SPAIR(  -7, 167), SPAIR(  13, 179), SPAIR(  47, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 45, 76);
const scorepair_t PawnAttacksRook   = SPAIR( 58, 54);
const scorepair_t PawnAttacksQueen  = SPAIR( 49, 11);
