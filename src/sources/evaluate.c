...
// Special eval terms
const scorepair_t CastlingBonus = SPAIR(98, -55);
const scorepair_t Initiative = SPAIR(25, 25);

// King Safety eval terms
const scorepair_t KnightWeight    = SPAIR(  52,  16);
...
const scorepair_t WeakKingZone    = SPAIR(  29, -22);
const scorepair_t SafeKnightCheck = SPAIR(  71,   7);
const scorepair_t SafeBishopCheck = SPAIR(  32,  29);
const scorepair_t SafeRookCheck   = SPAIR(  81,  40);
const scorepair_t SafeQueenCheck  = SPAIR(  49,  50);
const scorepair_t UnsafeCheck     = SPAIR(  17,  31);
const scorepair_t QueenlessAttack = SPAIR( -86,  -6);
const scorepair_t SafetyOffset    = SPAIR(  23,  35);
...
const scorepair_t KingStorm[24] = {
    SPAIR( -10,  -2), SPAIR( -12,  -1), SPAIR(  20,   4), SPAIR(  -2,   4),
    SPAIR( -25,  11), SPAIR( -20,   3), SPAIR( -45,   3), SPAIR( -15,   1),
    SPAIR(   0,   0), SPAIR( -19,  -5), SPAIR(  39,  10), SPAIR(   5,  -0),
    SPAIR(  -0,   2), SPAIR(  -4,  11), SPAIR(   6,  18), SPAIR(  15,  -1),
    SPAIR(  -1,  -1), SPAIR(  -2,  -0), SPAIR(  39,   3), SPAIR(  26,  -3),
    SPAIR(  -5,   2), SPAIR( -11,  15), SPAIR(  -5,  34), SPAIR(   2, -12)
};

const scorepair_t KingShelter[24] = {
    SPAIR( -42,   7), SPAIR( -33,  40), SPAIR( -33,  -9), SPAIR( -21,   5),
    SPAIR(  11,  -1), SPAIR(   7,  -0), SPAIR(   5,  -0), SPAIR(  -7, -21),
    SPAIR(   0,   0), SPAIR(  -6,   1), SPAIR(  -5,  31), SPAIR(   1,  14),
    SPAIR(  10,   6), SPAIR(  20,  -0), SPAIR(   7,  -0), SPAIR(  20, -17),
    SPAIR( -39,  -8), SPAIR(  11, -35), SPAIR(   6,  22), SPAIR(  12,  37),
    SPAIR(  12,   8), SPAIR(  23,   0), SPAIR(   6,   0), SPAIR(  15,  11)
};

// Knight eval terms
const scorepair_t KnightShielded      = SPAIR(  4, 17);
const scorepair_t KnightOutpost       = SPAIR( 12,-18);
const scorepair_t KnightCenterOutpost = SPAIR( 13, 12);
const scorepair_t KnightSolidOutpost  = SPAIR( 19, 24);

const scorepair_t ClosedPosKnight[5] = {
    SPAIR(   6, -18), SPAIR(   8,   2), SPAIR(   9,  18), SPAIR(  11,  29),
    SPAIR(  11,  47)
};

// Bishop eval terms
const scorepair_t BishopPairBonus    = SPAIR( 23,105);
const scorepair_t BishopShielded     = SPAIR(  2,  6);
const scorepair_t BishopLongDiagonal = SPAIR( 14, 38);

const scorepair_t BishopPawnsSameColor[7] = {
    SPAIR(  16,  40), SPAIR(  19,  31), SPAIR(  15,  16), SPAIR(  12,  10),
    SPAIR(   4,  -2), SPAIR(  -0,  -8), SPAIR(  -7, -15)
};

// Rook eval terms
const scorepair_t RookOnSemiOpenFile = SPAIR( 14, 31);
const scorepair_t RookOnOpenFile     = SPAIR( 30, 14);
const scorepair_t RookOnBlockedFile  = SPAIR( -5,-16);
const scorepair_t RookXrayQueen      = SPAIR( 15,  5);

// Mobility eval terms
const scorepair_t MobilityN[9] = {
    SPAIR( -58,  16), SPAIR( -45, -38), SPAIR( -32,  27), SPAIR( -24,  60),
    SPAIR( -18,  77), SPAIR( -12, 100), SPAIR(  -8, 108), SPAIR(  -0, 115),
    SPAIR(  -0, 109)
};

const scorepair_t MobilityB[14] = {
    SPAIR( -57, -41), SPAIR( -40, -44), SPAIR( -25, -13), SPAIR( -24,  20),
    SPAIR( -16,  39), SPAIR( -10,  52), SPAIR(  -5,  62), SPAIR(  -6,  66),
    SPAIR(  -7,  67), SPAIR(  -1,  63), SPAIR(  -3,  57), SPAIR(   7,  49),
    SPAIR(   6,  42), SPAIR(  13,  35)
};

const scorepair_t MobilityR[15] = {
    SPAIR(-111,  -4), SPAIR( -49,  18), SPAIR( -34,  63), SPAIR( -34,  85),
    SPAIR( -27, 107), SPAIR( -29, 126), SPAIR( -27, 132), SPAIR( -25, 137),
    SPAIR( -19, 145), SPAIR(  -8, 146), SPAIR(  -7, 157), SPAIR(  -0, 161),
    SPAIR(  10, 161), SPAIR(  15, 158), SPAIR(  29, 151)
};

const scorepair_t MobilityQ[28] = {
    SPAIR( -68,-156), SPAIR(  27, 165), SPAIR(   4, 161), SPAIR(   1, 137),
    SPAIR(   7,  89), SPAIR(  10, 120), SPAIR(   3, 147), SPAIR(   2, 172),
    SPAIR(   7, 190), SPAIR(  11, 203), SPAIR(   7, 217), SPAIR(   7, 221),
    SPAIR(  10, 227), SPAIR(  10, 232), SPAIR(  14, 233), SPAIR(   9, 239),
    SPAIR(  14, 239), SPAIR(  16, 233), SPAIR(  21, 225), SPAIR(  34, 217),
    SPAIR(  37, 209), SPAIR(  39, 198), SPAIR(  34, 190), SPAIR(  26, 177),
    SPAIR(  43, 154), SPAIR(  -7, 167), SPAIR(  14, 178), SPAIR(  46, 157)
};

// Threat eval terms
const scorepair_t PawnAttacksMinor  = SPAIR( 61, 85);
const scorepair_t PawnAttacksRook   = SPAIR( 63, 61);
const scorepair_t PawnAttacksQueen  = SPAIR( 51, 10);
const scorepair_t MinorAttacksRook  = SPAIR( 63, 49);
const scorepair_t MinorAttacksQueen = SPAIR( 45, 54);
