...
const scorepair_t BackwardPenalty  = SPAIR( -7, -9);
const scorepair_t StragglerPenalty = SPAIR(-18,-26);
const scorepair_t DoubledPenalty   = SPAIR(-22,-45);
const scorepair_t IsolatedPenalty  = SPAIR(-10,-14);

// Rank-based bonus for candidate passed Pawns
const scorepair_t CandidateBonus[8] = {
    0,
    SPAIR(-21,  2),
    SPAIR(-18,  8),
    SPAIR(-11, 38),
...
const scorepair_t PP_OurKingProximity[24] = {
    SPAIR(   4,  67), SPAIR(   5,  15), SPAIR(  -3, -75),
    SPAIR(   0,   0), SPAIR(   0,   0), SPAIR(   0,   0),
    SPAIR(   7,  69), SPAIR(   8,  24), SPAIR(  10, -27),
    SPAIR( -17, -53), SPAIR(   0,   0), SPAIR(   0,   0),
    SPAIR(   7,  65), SPAIR(  -3,  33), SPAIR( -12,  -9),
    SPAIR( -10, -35), SPAIR(  23, -41), SPAIR(   0,   0),
    SPAIR(  -2,  51), SPAIR( -16,  30), SPAIR( -12,  -8),
    SPAIR( -11, -13), SPAIR(  10, -18), SPAIR(  32, -25)
};

const scorepair_t PP_TheirKingProximity[24] = {
    SPAIR( -25,-142), SPAIR(   7,   8), SPAIR(  23, 143),
    SPAIR(   0,   0), SPAIR(   0,   0), SPAIR(   0,   0),
    SPAIR( -24,-142), SPAIR(  12, -25), SPAIR(  -1,  61),
    SPAIR(  22, 119), SPAIR(   0,   0), SPAIR(   0,   0),
    SPAIR( -10,-102), SPAIR(   8, -28), SPAIR(   7,   9),
    SPAIR(   6,  54), SPAIR(  -6,  80), SPAIR(   0,   0),
    SPAIR(  -8, -58), SPAIR( -11,  -9), SPAIR(  -7,   2),
    SPAIR(  14,   7), SPAIR(   6,  39), SPAIR(   3,  35)
};
    ...
    KingPawnEntry *entry, color_t us, bitboard_t ourPawns, bitboard_t theirPawns)
{
    ...
    while (ourPawns)
    {
        ...
        // Don't check for passers if an enemy Pawn resides on their queening path.
        if (queening & theirPawns)
            continue ;

        // Check if our Pawn has a free path to its queening square.
        if (!(entry->attackSpan[not_color(us)] & square_bb(sq)))
        {
        ...
        // For candidate passers, check if every square attacked by an enemy
        // Pawn is matched by a friendly Pawn attacking it too.
        else if ((queening & entry->attacks[not_color(us)] & ~entry->attacks[us]) == 0
            && (queening & entry->attacks2[not_color(us)] & ~entry->attacks2[us]) == 0)
        {
            ret += CandidateBonus[relative_sq_rank(sq, us)];
            TRACE_ADD(IDX_CANDIDATE + relative_sq_rank(sq, us) - RANK_2, us, 1);
        }
    }
