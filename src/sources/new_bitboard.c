...
// Returns a bitboard of all the reachable squares by a bishop/rook from the given square and board occupancy
Bitboard sliding_attacks_bb(const Direction *directions, Square square, Bitboard occupancy)
{
    ...
    for (u8 i = 0; i < 4; ++i)
    {
            ...
            sqi += directions[i])
        {
            ...
            if (bb_square_is_set(occupancy, sqi)) break;
        }
    }
...
void magic_init(Bitboard *attack_table, Magic *magic_table, const Direction directions[4])
{
    ...
    // The epoch is used to determine which iteration of the occupancy test we are in, to avoid having to zero the attack array between each failed iteration.
    ...
    for (Square square = SQ_A1; square <= SQ_H8; ++square)
    {
        ...
        // The edges of the board are not counted in the occupancy bits since we can reach them whether there's a piece on them or not because of capture moves, but we must still ensure they're accounted for if the piece is already on the edge for Rook moves.
        ...
        // We will need popcount(mask) bits of information for indexing the occupancy, and (1 << popcount(mask)) entries in the table for storing the corresponding attack bitboards.
        magic_entry->shift = 64 - bb_popcount(magic_entry->mask);
        magic_entry->moves = (square == SQ_A1) ? attack_table : magic_table[square - 1].moves + size;

        bb = 0;
        size = 0;

        // Iterate over all subsets of the occupancy mask with the Carry-Rippler trick and compute all attack bitboards for the current square based on the occupancy.
        do {
#ifndef USE_PEXT
            occupancy[size] = bb;
#endif
...
#ifdef USE_PEXT
            magic_entry->moves[_pext_u64(bb, m->mask)] = reference[size];
#endif
        ...
        // Now loop until we find a magic that maps each occupancy to a correct index in our magic table. We optimize the loop by using two binary ANDs to reduce the number of significant bits in the magic (good magics generally have high bit sparsity), and we reduce further the range of tested values by removing magics which do not generate enough significant bits for a full occupancy mask.
        for (u32 i = 0; i < size;)
        {
            for (magic_entry->magic = 0; u64_count_ones((magic_entry->magic * magic_entry->mask) >> 56) < 6;)
                magic_entry->magic = u64_random(&seed) & u64_random(&seed) & u64_random(&seed);

            // Check if the generated magic correctly maps each occupancy to its corresponding bitboard attack. Note that we build the table for the square as we test for each occupancy as a speedup, and that we allow two different occupancies to map to the same index if their corresponding bitboard attack is identical.
            for (++current_epoch, i = 0; i < size; ++i)
            {
                u32 index = magic_index(magic_entry, occupancy[i]);

                // Check if we already wrote an attack bitboard at this index during this iteration of the loop. If not, we can set the attack bitboard corresponding to the occupancy at this index; otherwise we check if the attack bitboard already written is identical to the current one, and if it's not the case, the mapping failed, and we must try another value.
...
void initialize_raw_slider_attacks(Piecetype piecetype)
{
    for (Square square = SQ_A1; square <= SQ_H8; ++square)
    {
        ...
        for (Square to = SQ_A1; to <= SQ_H8; ++to)
        {
            ...
            {
                LineBB[square][to] = attacks_bb(piecetype, square, 0) & attacks_bb(piecetype, square, 0);
