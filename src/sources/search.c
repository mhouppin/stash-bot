...
__main_loop:
    ...
    while ((currmove = movepicker_next_move(&mp, skipQuiets)) != NO_MOVE)
    {
        ...
        if (depth >= 3 && moveCount > 2 + 2 * pvNode)
        {
            ...
            // Perform a full-depth search if LMR failed high, and update
            // continuation histories based on whether it fails high or low.
            if (score > alpha && r != 0)
            {
                score = -search(
        ...
        // If LMR conditions aren't met, do a search with no reductions.
