...
f64 timeman_scale_score_diff(i32 score_diff) {
    const i32 k = 100;
    const f64 x = 2.0;

    // Clamp score diff to the range [-100, 100], and convert it to a time scale in
    // the range [0.5, 2.0].
    // Examples:
    // -100 -> 2.000x time
    //  -50 -> 1.414x time
    ...
    //  +50 -> 0.707x time
    // +100 -> 0.500x time
    return pow(x, i32_clamp(score_diff, -k, k) / (f64)k);
}
...
void timeman_update(
    ...
    if (timeman->previous_score != NO_SCORE) {
        scale *= timeman_scale_score_diff((i32)timeman->previous_score - (i32)root_score);
    }
