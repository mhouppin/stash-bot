...
inline unsigned int kpk_index(color_t stm, square_t bksq, square_t wksq, square_t psq)
{
    return (
            ...
            | ((unsigned int)bksq << 6)
            | ((unsigned int)stm << 12)
            | ((unsigned int)file_of_square(psq) << 13)
            | ((unsigned int)(RANK_7 - rank_of_square(psq)) << 15)
            );
}
...
inline bool kpk_is_winning(color_t stm, square_t bksq, square_t wksq, square_t psq)
{
    ...
    return (KPK_Bitbase[index / 8] & (1 << (index % 8)));
}
...
void    kpk_set(kpk_position_t *pos, unsigned int index)
{
    ...
    const square_t  psq = create_square((file_t)((index >> 13) & 0x3),
    ...
    else if (stm == WHITE && (PawnMoves[WHITE][psq] & square_bit(bksq)))
        pos->result = KPK_INVALID;

    // Can we promote without getting captured ?
    else if (stm == WHITE
            && rank_of_square(psq) == RANK_7
            && wksq != psq + NORTH
    ...
    else if (stm == BLACK && (king_moves(bksq) & ~king_moves(wksq) & square_bit(psq)))
        pos->result = KPK_DRAW;
...
void    kpk_classify(kpk_position_t *pos, kpk_position_t *kpk_table)
{
    ...
    {
        if (stm == WHITE)
            r |= kpk_table[kpk_index(BLACK, bksq, pop_first_square(&b), psq)].result;
        else
            r |= kpk_table[kpk_index(WHITE, pop_first_square(&b), wksq, psq)].result;
    }
    ...
    {
        // Single push
        if (rank_of_square(psq) < RANK_7)
            r |= kpk_table[kpk_index(BLACK, bksq, wksq, psq + NORTH)].result;

        // Double push
        if (rank_of_square(psq) == RANK_2 && psq + NORTH != wksq && psq + NORTH != bksq)
...
{
    square_t    winning_king = board_king_square(board, winning);
    square_t    winning_pawn = first_square(piecetype_bb(board, PAWN));
    square_t    losing_king = board_king_square(board, not_color(winning));
    color_t     us = winning == board->side_to_move ? WHITE : BLACK;

    winning_king = normalize_square(board, winning, winning_king);
    winning_pawn = normalize_square(board, winning, winning_pawn);
    losing_king = normalize_square(board, winning, losing_king);

    score_t     score = kpk_is_winning(us, losing_king, winning_king, winning_pawn)
        ? VICTORY + PAWN_EG_SCORE + rank_of_square(winning_pawn) * 3 : 1;

    return (winning == board->side_to_move ? score : -score);
}
