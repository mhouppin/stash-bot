...
void trace_psqt(const Board *board) {
    for (Bitboard occ = board_occupancy_bb(board); occ != 0;) {
        const Square sq = bb_pop_first_square(&occ);
        const Piece pc = board_piece_on(board, sq);
        const Piecetype pt = piece_type(pc);
        const Color c = piece_color(pc);

        if (pt == PAWN) {
            trace_add(IDX_PSQT + square_relative(sq, c) - SQ_A2, c, 1);
        } else {
            trace_add(
                IDX_PSQT + square_to_psqt_index(square_relative(sq, c)) + (pt - KNIGHT) * 32 + 48,
                c,
                1
            );
        }
        ...
        if (pt != KING) {
            trace_add(IDX_PIECE + pt - PAWN, c, 1);
        }
    }
}
