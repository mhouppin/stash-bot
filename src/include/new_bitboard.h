...
// This function returns the index of the attack bitboard for a given magic and occupancy bitboard. Avoid using this function directly, use the helpers for bishop/rook/queen attacks instead.
INLINED u32 magic_index(const Magic *magic, Bitboard occupancy)
{
#ifdef USE_PEXT
...
INLINED Bitboard bb_shift_up(Bitboard bb) { return bb << 8; }

// Shifts the bitboard down by one rank
INLINED Bitboard bb_shift_down(Bitboard bb) { return bb >> 8; }

// Shifts the bitboard left by one file
INLINED Bitboard bb_shift_left(Bitboard bb) { return (bb & ~FILE_A_BB) >> 1; }

// Shifts the bitboard right by one file
INLINED Bitboard bb_shift_right(Bitboard bb) { return (bb & ~FILE_H_BB) << 1; }

// Shifts the bitboard up and left
INLINED Bitboard bb_shift_up_left(Bitboard bb) { return (bb & ~FILE_A_BB) << 7; }

// Shifts the bitboard up and right
INLINED Bitboard bb_shift_up_right(Bitboard bb) { return (bb & ~FILE_H_BB) << 9; }

// Shifts the bitboard down and left
INLINED Bitboard bb_shift_down_left(Bitboard bb) { return (bb & ~FILE_A_BB) >> 9; }

// Shifts the bitboard down and right
INLINED Bitboard bb_shift_down_right(Bitboard bb) { return (bb & ~FILE_H_BB) >> 7; }
...
INLINED bool bb_more_than_one(Bitboard bb) { return bb & (bb - 1); }
...
{
    HINT(square_is_valid(square1));
    HINT(square_is_valid(square2));
