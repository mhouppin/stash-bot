...
INLINED bool is_valid_move(Move move)
{
...
{
    HINT(square_is_valid(from));
    HINT(square_is_valid(to));
...
INLINED Move create_promotion_move(Square from, Square to, Piecetype piecetype)
{
    HINT(square_is_valid(from));
    HINT(square_is_valid(to));
    HINT(piecetype >= KNIGHT && piecetype <= QUEEN);
...
{
    HINT(score_is_usable(midgame));
    HINT(score_is_usable(endgame));
    return (Scorepair)(((u32)(i32)midgame << 16) + (u32)(i32)endgame);
}

INLINED Scorepair scorepair_multiply(Scorepair scorepair, i16 factor)
{
    HINT(i32_abs(scorepair_midgame(scorepair) * factor) < INF_SCORE);
    HINT(i32_abs(scorepair_endgame(scorepair) * factor) < INF_SCORE);
...
{
    HINT(factor != 0);
    return create_scorepair(scorepair_midgame(scorepair) / factor, scorepair_endgame(scorepair) / factor);
}
