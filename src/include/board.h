...
// Sets a castling right from the given color and rook square.
...
INLINED piece_t piece_on(const board_t *board, square_t square) { return (board->table[square]); }
...
INLINED bitboard_t color_bb(const board_t *board, color_t color) { return (board->colorBB[color]); }
...
INLINED bitboard_t occupancy_bb(const board_t *board) { return (piecetype_bb(board, ALL_PIECES)); }
